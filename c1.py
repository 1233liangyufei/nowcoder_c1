...
def demo_random():
    ...
    print 4,random.sample(range(1,100),4)
    a = [1,2,3,4,5]
    random.shuffle(a)
    print 5,a
...
#    p4 = re.compile('<h>([^<]+)<body>([]+)</body>')

    str = 'xx2016-06-11yy'
    p5 = re.compile('\d\d\d\d-\d\d-\d\d')
    p6 = re.compile('\d{4}-\d{2}-\d{2}')
    print 2,p5.findall(str)
    print 3,p6.findall(str)
